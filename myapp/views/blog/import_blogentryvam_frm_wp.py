...
def home(request):
    ...
    newhost="http://vmanival.eu.pythonanywhere.com/static/uploads/legacy_wp/"
    ...
    for item in items:
        ...
        imgurl=""
        imgnewurl=""
        re1=re.compile("https:\/\/vivreaumanival\.files\.wordpress.com\/(.*?)png")
        imgs=re1.findall(content)
        for imgurl in imgs:
            if imgurl:
                re2=re.compile('([^/]+$)')
                img2=re2.search(imgurl)
                imgfn=img2.group()+"png"
                imgnewurl= host+imgfn
                content=content.replace(legacy+imgurl+".png",imgnewurl)
        """
        # --------------------------------------------------
